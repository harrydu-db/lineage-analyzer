...
class ETLLineageAnalyzer:
    ...
    def __init__(self):
        self.sql_keywords = {
            'CREATE', 'INSERT', 'UPDATE', 'SELECT', 'FROM', 'JOIN', 'LEFT', 'RIGHT', 'INNER', 'OUTER',
            'WHERE', 'AND', 'OR', 'IN', 'EXISTS', 'UNION', 'CASE', 'WHEN', 'THEN', 'ELSE', 'END',
            'GROUP', 'BY', 'ORDER', 'HAVING', 'DISTINCT', 'COALESCE', 'NULL', 'AS', 'ON', 'BT', 'ET',
            'WITH', 'DATA', 'ON', 'COMMIT', 'PRESERVE', 'ROWS', 'SEL', 'DISTINCT', 'CASE', 'WHEN',
            'CHARACTERS', 'TRIM', 'SUBSTR', 'SUBSTRING', 'CURRENT_TIMESTAMP', 'CAST', 'COALESCE'
        }
        
        # Common single-letter aliases to ignore
        self.common_aliases = {'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 
    ...
    def is_valid_table_name(self, table_name: str) -> bool:
        ...
        if any(keyword in table_name.upper() for keyword in ['SELECT', 'CASE', 'WHEN', 'THEN', 'ELSE', 'END']):
            return False
    ...
    def extract_sql_blocks(self, content: str) -> List[str]:
        ...
        if 'bteq' in content.lower() or '<<EOF' in content:
            ...
            sql_content = re.sub(r'^#!.*$', '', sql_content, flags=re.MULTILINE)  # Shebang
            sql_content = re.sub(r'^#.*$', '', sql_content, flags=re.MULTILINE)   # Shell comments
            sql_content = re.sub(r'^\..*$', '', sql_content, flags=re.MULTILINE)  # Shell dot commands
    ...
    def extract_table_names(self, sql_block: str) -> Set[str]:
        ...
        create_matches = re.finditer(create_pattern, sql_clean, re.IGNORECASE | re.DOTALL)
        ...
        subquery_matches = re.finditer(subquery_pattern, sql_clean, re.IGNORECASE | re.DOTALL)
    ...
    def _parse_sql_statement(self, statement: str, line_number: int) -> Optional[TableOperation]:
        """Parse a single SQL statement and extract operation info"""
        statement = statement.strip()
        
        # CREATE VOLATILE TABLE
        create_match = re.search(r'CREATE\s+VOLATILE\s+TABLE\s+(\w+)\s+AS\s*\(', statement, re.IGNORECASE | re.DOTALL)
        if create_match:
        ...
        update_match = re.search(r'UPDATE\s+\w+\s+FROM\s+([\w\.]+)', statement, re.IGNORECASE)
        if update_match:
        ...
        for token in tokens:
            if from_seen:
                if self._is_identifier(token):
                    ...
                    if hasattr(token, 'is_group') and token.is_group and any(t.ttype is DML and t.value.upper() == 'SELECT' for t in token.tokens):
                    ...
                    for identifier in token.get_identifiers():
                        if hasattr(identifier, 'is_group') and identifier.is_group and any(t.ttype is DML and t.value.upper() == 'SELECT' for t in identifier.tokens):
                            tables.extend(self._extract_tables_from_tokenlist(identifier.tokens))
                        else:
                            name = self._get_identifier_name(identifier)
                            if name:
                                tables.append(name)
                    from_seen = False
                elif token.ttype is Keyword and token.value.upper() in ("SELECT", "WHERE", "GROUP", "ORDER", "HAVING", "UNION", ")"):
                    from_seen = False
            if token.is_group:
                tables.extend(self._extract_tables_from_tokenlist(token.tokens))
            if token.ttype is Keyword and token.value.upper() in ("FROM", "JOIN", "INNER JOIN", "LEFT JOIN", "LEFT OUTER JOIN", "RIGHT JOIN", "RIGHT OUTER JOIN"):
                from_seen = True
        return tables

    def _is_identifier(self, token):
        return isinstance(token, Identifier) or (token.ttype is None and hasattr(token, 'get_real_name'))
    ...
    def analyze_script(self, script_path: str) -> LineageInfo:
        ...
        all_tables = self.extract_table_names(combined_sql)
    ...
    def export_to_json(self, lineage_info: LineageInfo, output_file: str = None):
        ...
        data = {
            ...
            "table_relationships": {
                target: sources for target, sources in lineage_info.table_relationships.items()
            }
        }
...
"""
    ...
    def process_folder(self, input_folder: str, output_folder: str):
        ...
        script_files = list(input_path.glob("*.sh")) + list(input_path.glob("*.ksh")) + list(input_path.glob("*.sql"))
        ...
        for script_file in script_files:
            try:
                ...
                json_file = output_path / f"{script_file.stem}_{script_file.suffix[1:]}_lineage.json"
                self.export_to_json(lineage_info, str(json_file))
                
                # Generate HTML report with extension included
                html_file = output_path / f"{script_file.stem}_{script_file.suffix[1:]}_lineage.html"
...
Examples:
  ...
  python lineage.py BatchTrack.sh --json
    ...
    parser.add_argument(
        '--export',
        help='Export lineage data to JSON file (for single file mode)'
    )
